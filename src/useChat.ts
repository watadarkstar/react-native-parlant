...
import type { IMessage, Props, Session, MessageEvent } from "./types";
...
export const useChat = ({
  ...
  const sendMessage = async (message: string) => {
    ...
    const response = await fetch(
      `${_api}/sessions/${_sessionId}/events?moderation=${_moderation}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const json: MessageEvent = await response.json();
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      while (true) {
        const response = await fetch(
          `${_api}/sessions/${sessionId}/events?min_offset=${minOffset.current}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!response.ok) {
          ...
          if (retries.current < _maxRetries) {
            retries.current += 1;
            console.warn(
              `Retrying fetch events (${retries.current}/${_maxRetries})...`
            );
            continue;
          } else {
            break;
        ...
        const data: MessageEvent[] = await response.json();
        ...
        if (data.length > 0) {
          ...
          if (newMessages.length > 0) {
            setMessages((previousMessages) =>
              append(previousMessages, newMessages)
            );
          }
 ...
 */
export const append = <TMessage extends IMessage>(
